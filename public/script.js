...
function collap(room_id) {
  ...
  if (height == "0") {
    ...
    $("#" + room_id + "-msg")
      .find(".Participants")
      .css({ opacity: "0", "z-index": "-10" });
    $("#" + room_id + "-msg")
      .find(".btn")
      .removeClass("viewUsers");
  }
}
...
function writeEmoji(emoji) {
  ...
  const $textarea = $(`.right.active #${emoji.id}`)
    .parents(".right.active")
    .find(".textarea");
...
function SidebarToggle() {
  const navbar = document.querySelector(".left");
  if (
    !navbar.classList.contains("menu-closed") &&
    navbar.classList.contains("open-menu")
  ) {
    closeSidebar();
  } else {
    openSidebar();
    document.body.addEventListener(
      ...
      function (event) {
        ...
        if (
          ...
          !addLobbyPage.contains(event.target)
        ) {
          closeSidebar();
        }
      },
