...
function leaveRoom(room) {
  ...
  $(".error").html(
    '<span id="error">You haven\'t joined this room yet. <a onclick="joinRoom( \'' +
      room +
      '\' )" id="joinBtn" href="#">Join</a> to see the conversation.</span>'
  );
...
socket.on("user exists", (data) => {
  ...
  nameError.innerHTML =
    "There is already one person with this nickname, try another one.";
});
...
socket.on("user set", function (data) {
  ...
  $(".top[data-chat='person1']").find("span")[1].innerHTML =
    data.online + " user(s) online</span>";
  $(".Participants").find("span")[0].innerHTML = convertIntoList(
    data.online_users
  );
...
socket.on("user joined", function (data) {
  ...
  $("#lobby-msg").find(".top").find("span")[1].innerHTML =
    data.online + " user(s) online";
  $(".Participants").find("span")[0].innerHTML = convertIntoList(
    data.online_users
  );
});
...
socket.on("user join", function (data) {
  ...
  if (data.room != "lobby") {
    ...
    $("#" + room_id + "-msg")
      .find(".top")
      .find("span")[1].innerHTML = data.online + " user(s) online";
    $("#" + room_id + "-msg")
      .find(".Participants")
      .find("span")[0].innerHTML = convertIntoList(data.online_users);
  }
});
...
socket.on("Display Message", function (data) {
  ...
  var room_id = convertIntoId(data.room);
  $("#" + room_id + "-msg")
    .children(".chat[data-chat='person1']")
    .append(div);
  ...
  if (socket.username != data.user && currRoom != data.room && isJoined == 1) {
    ...
    $.notify(
      `Room ${data.room}-\n${data.user}: ${
        p_notif.length >= 20 ? p_notif.substr(0, 20) + "..." : p_notif
      }`,
      "info"
    );
  }
...
socket.on("room created self", function (data) {
  ...
  $(`#${room_id}-msg`)
    .find(".Participants")
    .find("span")[0].innerHTML = convertIntoList(online_users);
  $("#room").fadeOut();
  $(".wrapper").fadeIn();
  $("#roomName").val("");
  $("#description").val("");
...
socket.on("update info", function (rooms) {
  ...
  for (var i = 0; i < rooms.length; i++) {
    ...
    $("#" + room_id + "-msg")
      .find(".top")
      .find("span")[1].innerHTML = rooms[i].num_users + " user(s) online";
    $("#" + room_id + "-msg")
      .find(".Participants")
      .find("span")[0].innerHTML = convertIntoList(rooms[i].users);
  }
});
...
socket.on("room joined", function (data) {
  ...
  $("#" + room_id + "-msg")
    .find(".top")
    .find("span")[1].innerHTML = online + " user(s) online";
  $("#" + room_id + "-msg")
    .find(".Participants")
    .find("span")[0].innerHTML = convertIntoList(online_users);
});
