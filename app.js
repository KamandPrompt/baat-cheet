...
io.on('connection', (socket) => {
  ...
  socket.on('set username', (name) => {
    ...
    //if username is not taken
      ...
      if (rooms[0].users.indexOf(name) == -1) {
        ...
        socket.broadcast.emit('user joined',
          {username: name, online: rooms[0].num_users, online_users: rooms[0].users});
  ...
  socket.on('create room', (data) => {
    ...
    socket.emit('room created self',
      {room_name: data.room_name, description: data.description, online: 1, online_users: [socket.username]});
    socket.broadcast.emit('room created other',
      {room_name: data.room_name, description: data.description, online_users: [socket.username]});
  });
  ...
  socket.on('join room', (room) => {
    ...
    if (fetchedRoom !== undefined) {
      ...
      socket.emit('room joined', {name: room.name, online: fetchedRoom.num_users, online_users: fetchedRoom.users});

      //notify other users in room that someone joined
      socket["to"](room.name).broadcast.emit('user join', {
        username: socket.username, room: room.name, online: fetchedRoom.num_users, online_users: fetchedRoom.users
      });
    }
  });
  ...
  socket.on('disconnecting', () => {
    ...
    const filteredRoom = rooms.find(eachRoom => socket.rooms[eachRoom.name]);
